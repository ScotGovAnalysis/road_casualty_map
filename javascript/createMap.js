// Javascript code not standalone but to be run through R
// Using R htmlwidgets::onRender

function(el, x, data) {
    // To set the icon colour based on severity of accident
    ...
    // To get icon type depending on road user
    function getIcon(input_type, sev) {
        const iconType = {
            'Pedestrians': L.AwesomeMarkers.icon({
                icon: 'android-walk',
                prefix: 'ion',
                markerColor: getColor(sev),
                iconColor: 'black'
            }),
            'Pedal cyclists': L.AwesomeMarkers.icon({
                icon: 'bicycle',
                prefix: 'fa',
                markerColor: getColor(sev),
                iconColor: 'black'
            }),
            'Motorcyclists': L.AwesomeMarkers.icon({
                icon: 'motorcycle',
                prefix: 'fa',
                markerColor: getColor(sev),
                iconColor: 'black'
            }),
            'Cars and taxis': L.AwesomeMarkers.icon({
                icon: 'car',
                prefix: 'fa',
                markerColor: getColor(sev),
                iconColor: 'black'
            }),
            'Bus, Coach, Minibus': L.AwesomeMarkers.icon({
                icon: 'bus',
                prefix: 'fa',
                markerColor: getColor(sev),
                iconColor: 'black'
            }),
            'LGV and HGV': L.AwesomeMarkers.icon({
                icon: 'truck',
                prefix: 'fa',
                markerColor: getColor(sev),
                iconColor: 'black'
    ...
    //Make the clusters and subgroups
    ...
    // Iterate through the R data frame and create markers and assign to groups
    for (var i = 0; i < data.longitude.length; i++) {
        marker = L.marker([data.latitude[i], data.longitude[i]], {
            title: data.road_user[i],
            icon: getIcon(data.road_user[i], data.CASSEV[i])
        });
